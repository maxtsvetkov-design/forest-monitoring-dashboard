...
export default defineConfig(({ mode }) => {
  ...
  return {
    // GITHUB_PAGES_BASE is set by .github/workflows/deploy.yml for the Pages
    // build only — a project page is served from a subpath
    // (username.github.io/repo-name/), so every asset URL the build emits
    // needs that prefix baked in. Local dev and Figma Make previews never
    // set it, so they keep resolving from '/' as before.
    ...
    optimizeDeps: {
      // maplibre-gl spawns a web worker whose URL depends on its own
      // (unbundled) module location; letting esbuild pre-bundle it breaks
      // that URL resolution and the worker fails to load.
      exclude: ['maplibre-gl'],
    },
    server: {
      host: '0.0.0.0',
...
/**
 * MapLibre resolves its worker script at runtime as a sibling of whatever
 * URL its own module ends up loaded from (`new URL('maplibre-gl-worker.mjs',
 * import.meta.url)`), rather than a static `import` Vite's bundler can see
 * and follow. `optimizeDeps.exclude` above keeps it unbundled for *dev*, so
 * the dev server can serve that sibling file straight out of node_modules --
 * but a production build inlines maplibre-gl's code into the app's own
 * hashed chunk, and nothing then copies the actual worker file into
 * `dist/assets/` next to it. The result is a 404 on the worker and a blank
 * map in any real production build (GitHub Pages included), never caught by
 * `vite dev`. This copies both the prod and dev worker builds into the same
 * `assets/` directory Vite emits chunks into, build-only. The worker in turn
 * pulls in its own sibling (`maplibre-gl-shared(-dev).mjs`) the same
 * runtime-relative-URL way, so every loose `maplibre-gl-*.mjs` file next to
 * the package's main entry gets copied, not just the worker itself.
 */
function maplibreWorkerPlugin(): Plugin {
  return {
    ...
    generateBundle() {
      ...
      for (const fileName of fs.readdirSync(maplibreDist)) {
        // Only the loose runtime-loaded siblings -- not maplibre-gl(-dev).mjs
        // itself, which the bundler already inlines into the app's own chunk.
        if (!/^maplibre-gl-(worker|shared)(-dev)?\.mjs$/.test(fileName)) continue
        this.emitFile({
          type: 'asset',
          fileName: `assets/${fileName}`,
          source: fs.readFileSync(path.join(maplibreDist, fileName)),
        })
      }
    },
  }
